...
def test_run_turn_with_model_flag():
    ...
    connector = ClaudeCliConnector(
        claude_command="claude", model="claude-sonnet-4-6", inject_tools_context=False
    )
...
def test_run_turn_timeout():
    ...
    timeout_proc = _make_mock_proc(communicate_side_effect=subprocess.TimeoutExpired("claude", 1.0))
