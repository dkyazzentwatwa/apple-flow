...
- `apple_flow_file_aliases` -- JSON alias map for `@f:<alias>` prompt references to common files
...
Voice message + TTS controls (used by approval-gated `voice:` and `voice-task:` audio iMessages) are configured via:
