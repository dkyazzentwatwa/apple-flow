...
class ClineConnector:
    ...
    def _parse_json_output(self, raw_output: str) -> str:
        ...
        {"type": "say"|"ask", "text": "...", "say": "text"|..., "ts": ...}

        We collect all "say" messages with say="text" to build the response.
        """
        text_parts: list[str] = []
        ...
        for line in raw_output.splitlines():
            ...
            # Only collect non-partial "say" messages with subtype "text"
            if (
                obj.get("type") == "say"
                and obj.get("say") == "text"
                and not obj.get("partial", False)
            ):
        ...
        if text_parts:
            # Return the last substantive text message as the final response
            return text_parts[-1]
    ...
    def run_turn(self, thread_id: str, prompt: str) -> str:
        ...
        logger.info(
            "Executing Cline CLI: sender=%s workspace=%s timeout=%.1fs model=%s context_items=%d",
            sender,
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                logger.error(
                    "Cline exec failed: returncode=%d stderr=%s",
                    result.returncode,
                    error_msg,
                )
                return f"Error: Cline execution failed (exit code {result.returncode}). Check logs for details."
